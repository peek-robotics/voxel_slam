...
enum LID_TYPE
{
    ...
    TARTANAIR,
    LIVOX_POINTCLOUD2,
    GENERIC_XYZI // Generic PointCloud2 with XYZ + intensity, no timing
};
...
public:
    ...
    // Generic handler for PointCloud2 with XYZ + intensity only (no per-point time)
    // Useful for aggregated/local accumulated clouds from other odometry nodes like Faster-LIO.
    // For timing-less clouds, set Odometry/point_notime=1 so the pipeline doesn't rely on curvature.
    void generic_xyzi_handler(const sensor_msgs::PointCloud2::ConstPtr& msg, pcl::PointCloud<PointType>& pl_full)
    {
        ...
        for (int i = 0; i < plsize; ++i)
        {
            const auto& iter = pl_orig.points[i];
            PointType ap;
            ap.x = iter.x;
            ap.y = iter.y;
            ap.z = iter.z;
