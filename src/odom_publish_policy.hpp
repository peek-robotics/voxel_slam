...
struct OdomPublishPolicy
{
  ...
  double pose_absolute = 0.0; // >0: replace the diagonal, else scale
...
inline OdomPublishPolicy decideOdomPublish(bool anchored, bool initializing,
                                           DegradeState degrade,
                                           int frames_since_discontinuity,
                                           int discontinuity_frames,
                                           double discontinuity_variance,
                                           double uncertain_variance)
{
  ...
  {
    p.pose_absolute = discontinuity_variance;
  ...
  {
    p.pose_absolute = uncertain_variance;
...
// Applies one of the two regimes to a row-major 6x6 covariance array.
template <typename Cov>
inline void applyOdomCovariance(Cov &cov, double absolute, double scale,
                                bool zero_off_diagonal)
{
  if (absolute > 0.0)
  {
    for (int i = 0; i < 6; ++i)
      for (int j = 0; j < 6; ++j)
        cov[i * 6 + j] = (i == j) ? absolute : (zero_off_diagonal ? 0.0 : cov[i * 6 + j]);
    return;
  }
  if (scale != 1.0)
    for (int k = 0; k < 36; ++k)
