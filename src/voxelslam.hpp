...
ros::Publisher pub_prev_path, pub_curr_path;
ros::Publisher pub_odom;
ros::Publisher pub_local_accum_cloud; // publishes local accumulated cloud frames
ros::Publisher pub_scan_full;         // full-resolution deskewed LiDAR-frame scan (no downsampling)
...
bool pub_local_accumulated = false; // if true, publish our own local accumulated cloud
std::string local_accumulated_topic = "local_accumulated";
bool enable_local_mapping = true; // if false, skip local mapping and use external odom TF
bool enable_loop_closure = true;  // if false, do not run loop closure/global mapping threads
bool use_external_odom_tf = false; // if true and local mapping disabled, use TF(odom->base_link) each cycle
